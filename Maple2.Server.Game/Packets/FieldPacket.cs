...
public static class FieldPacket {
    public static ByteWriter AddPlayer(GameSession session) {
        ...
        IEnumerable<Buff> buffs = session.Player.Buffs.GetBuffs();
    ...
    public static ByteWriter AddNpc(FieldNpc npc) {
        ...
        IEnumerable<Buff> buffs = npc.Buffs.GetBuffs();
        pWriter.WriteShort((short) buffs.Count());
        foreach (Buff buff in buffs) {
            pWriter.WriteInt(buff.Owner.ObjectId);
            pWriter.WriteInt(buff.ObjectId);
        ...
        if (npc.Value.IsBoss) {
            ...
            pWriter.WriteInt(npc.Value.Metadata.Property.Skills.Length);
            foreach (NpcMetadataSkill skill in npc.Value.Metadata.Property.Skills) {
                pWriter.WriteInt(skill.Id);
                pWriter.WriteShort((short) skill.Level);
            }
